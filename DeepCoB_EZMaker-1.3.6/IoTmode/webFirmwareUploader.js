 ...
 * 
 * === 단계별 업그레이드 시스템 ===
 * 1. runStep1Upload() - 1단계: 새 파일들을 temp 디렉토리에 저장
 * 2. runStep2Backup() - 2단계: 기존 파일들을 backup 디렉토리에 백업
 * 3. runStep3Apply() - 3단계: temp 파일들을 실제 위치에 적용
 * 4. runStep4Cleanup() - 4단계: temp 파일 정리 및 시스템 재시작
 * 
 * === 추가 기능 ===
 * - checkUpgradeStatus() - ESP32 상태 확인
 * - rollbackFirmware() - 백업 파일로 이전 버전 복원
 * 
 * === BLE 통신 개선사항 ===
 * - BLEManager 중앙화된 ACK 수신 구조 활용
 * - 이중 안전장치 (onDataReceived + startNotifications)
 * - 향상된 데이터 형식 처리 및 오류 추적
 */
class WebFirmwareUploader {
    constructor() {
        ...
        // 시스템 정보 임시 저장
        this._currentSystemMemory = null;
        this._currentFilesystemInfo = null;
    ...
    async _processZipFile(zipFile) {
        ...
        console.log(`[ZIP] ZIP 파일 처리 시작: ${zipFile.name}`);
        ...
        for (const [filename, fileData] of Object.entries(zipData.files)) {
            if (!fileData.dir && (filename.endsWith('.mpy') || filename.endsWith('.py'))) {
                ...
                const fileInfo = {
                    filename: filename,
                    size: fileSize,
                    type: filename.endsWith('.mpy') ? 'MicroPython Bytecode' : 'Python Source',
                    path: filename.startsWith('lib/') ? 'Library' : 'Root'
                };
                fileList.push(fileInfo);
                totalFiles++;
                totalSize += fileSize;
                
                console.log(`[ZIP] 파일 추가: ${filename} (${fileSize} bytes, ${fileInfo.type})`);
            }
        }
        ...
        if (fileList.length > 0) {
            ...
            let fileListContent = `# ESP32-S3 펌웨어 패키지 파일 리스트\n`;
            fileListContent += `# 생성 시간: ${timestamp}\n`;
            fileListContent += `# 소스 ZIP: ${zipFileName}\n`;
            fileListContent += `# 총 파일 수: ${totalFiles}개\n`;
            fileListContent += `# 총 크기: ${totalSize} bytes (${(totalSize / 1024).toFixed(1)} KB)\n`;
            fileListContent += `\n`;
            
            // 파일 유형별 분류
            const libFiles = fileList.filter(f => f.path === 'Library');
            const rootFiles = fileList.filter(f => f.path === 'Root');
            
            if (rootFiles.length > 0) {
                fileListContent += `## 루트 디렉토리 파일 (${rootFiles.length}개)\n`;
                rootFiles.forEach(file => {
                    fileListContent += `${file.filename.padEnd(30)} ${file.size.toString().padStart(8)} bytes  ${file.type}\n`;
                });
                fileListContent += `\n`;
            }
            
            if (libFiles.length > 0) {
                fileListContent += `## 라이브러리 파일 (${libFiles.length}개)\n`;
            ...
            // 상세 정보
            fileListContent += `## 전체 파일 리스트\n`;
            fileList.forEach((file, index) => {
                fileListContent += `${(index + 1).toString().padStart(2)}. ${file.filename}\n`;
                fileListContent += `    크기: ${file.size} bytes\n`;
                fileListContent += `    유형: ${file.type}\n`;
                fileListContent += `    경로: ${file.path}\n`;
                fileListContent += `\n`;
            });
            
            // file_list.txt를 텍스트 파일로 변환하여 펌웨어 패키지에 추가
            const fileListBuffer = new TextEncoder().encode(fileListContent);
            this.addFirmwareFile('file_list.txt', fileListBuffer);
            
            console.log(`[ZIP] 📋 file_list.txt 생성 완료 (${fileListBuffer.length} bytes)`);
            console.log(`[ZIP] 파일 리스트 내용:\n${fileListContent}`);
            
            // 상태 메시지 출력 (HTML 페이지에서 확인 가능)
            if (this.statusCallback) {
                this.statusCallback(`📋 파일 리스트 생성: ${totalFiles}개 파일, 총 ${(totalSize / 1024).toFixed(1)}KB`);
                this.statusCallback(`📁 루트 파일: ${rootFiles.length}개, 라이브러리 파일: ${libFiles.length}개`);
            }
        }
        
        console.log(`[ZIP] ZIP 파일 처리 완료: 총 ${fileList.length}개 파일 추가`);
    }
    ...
    /**
     * 🔬 단계별 테스트: 1단계 - temp 저장만 실행 (commit 하지 않음)
     */
    async runStep1Upload() {
    ...
    async checkFirmwareVersion() {
        console.log('[WebFirmwareUploader] === ESP32 펌웨어 버전 확인 시작 ===');
        
        try {
            this._updateStatus('ESP32 펌웨어 버전 확인 중...');
            
            // === BLE 재연결 시 ACK 핸들러 재등록 보장 ===
            if (this.bleManager && this.bleManager.isConnected) {
                console.log('[WebFirmwareUploader] 펌웨어 버전 확인 전 ACK 핸들러 강제 재등록');
                this._registerAckHandler(true); // 강제 재등록
            ...
            // UPGRADE:FIRMWARE_VERSION 명령 전송
            await this._sendCommand({
                command: 'UPGRADE:FIRMWARE_VERSION',
                channel: 'upgrade'
            });
            
            console.log('[WebFirmwareUploader] === 펌웨어 버전 확인 명령 전송 완료 ===');
            
        } catch (error) {
            this._updateError('펌웨어 버전 확인 실패: ' + error.message);
            throw error;
        }
    }
    ...
    _handleStatusResponse(message) {
        try {
            ...
            // === 🔍 펌웨어 버전 관련 메시지 처리 ===
            else if (message.startsWith('FIRMWARE_VERSION_CHECK_START')) {
                console.log('🔍 [STATUS_DEBUG] FIRMWARE_VERSION_CHECK_START 처리');
                this._updateStatus('🔍 펌웨어 버전 확인 시작...', 'info');
            }
            else if (message.startsWith('FIRMWARE_VERSION:')) {
                // FIRMWARE_VERSION:1.3.5
                const version = message.substring(17); // 'FIRMWARE_VERSION:' 제거
                console.log(`🔍 [STATUS_DEBUG] 펌웨어 버전 추출: "${version}"`);
                this._updateStatus(`📋 현재 펌웨어 버전: ${version}`, 'success');
                
                // 화면에 펌웨어 버전 표시 (HTML의 showFirmwareVersion 함수 호출)
                if (typeof window.showFirmwareVersion === 'function') {
                    // 시스템 정보 수집
                    const systemInfo = {};
                    if (this._currentSystemMemory) {
                        systemInfo.memory = this._currentSystemMemory;
                    }
                    if (this._currentFilesystemInfo) {
                        systemInfo.filesystem = this._currentFilesystemInfo;
                    }
                    
                    window.showFirmwareVersion(version, systemInfo);
                }
                
                // 추가 정보 표시
                if (version !== 'unknown') {
                    this._updateStatus(`✅ config.py에서 버전 정보를 성공적으로 읽었습니다`, 'info');
                } else {
                    this._updateStatus(`⚠️ 버전 정보를 읽을 수 없습니다`, 'warning');
                    
                    // 오류 상태로 화면 표시
                    if (typeof window.showFirmwareVersionError === 'function') {
                        window.showFirmwareVersionError('config 파일 읽기 실패');
                    }
                }
            }
            else if (message.startsWith('FIRMWARE_VERSION_ERROR:')) {
                const error = message.substring(24); // 'FIRMWARE_VERSION_ERROR:' 제거
                this._updateStatus(`❌ 펌웨어 버전 확인 오류: ${error}`, 'error');
            }
            else if (message.startsWith('FIRMWARE_VERSION_WARNING:')) {
                const warning = message.substring(26); // 'FIRMWARE_VERSION_WARNING:' 제거
                this._updateStatus(`⚠️ 펌웨어 버전 경고: ${warning}`, 'warning');
            }
            else if (message.startsWith('SYSTEM_MEMORY:')) {
                // SYSTEM_MEMORY:123456
                const memory = parseInt(message.substring(14));
                this._currentSystemMemory = memory; // 임시 저장
                const memoryKB = (memory / 1024).toFixed(1);
                this._updateStatus(`🧠 시스템 메모리: ${memoryKB}KB 사용 가능`, 'info');
            }
            else if (message.startsWith('FILESYSTEM_TOTAL:')) {
                const total = parseInt(message.substring(18));
                const totalKB = (total / 1024).toFixed(1);
                this._updateStatus(`💾 파일시스템 총 용량: ${totalKB}KB`, 'info');
            }
            else if (message.startsWith('FILESYSTEM_FREE:')) {
                const free = parseInt(message.substring(17));
                // 파일시스템 정보 임시 저장
                if (!this._currentFilesystemInfo) {
                    this._currentFilesystemInfo = {};
                }
                this._currentFilesystemInfo.free = free;
                
                const freeKB = (free / 1024).toFixed(1);
                this._updateStatus(`💿 파일시스템 사용 가능: ${freeKB}KB`, 'info');
            }
            else if (message === 'FIRMWARE_VERSION_CHECK_COMPLETE') {
                this._updateStatus('✅ 펌웨어 버전 확인 완료', 'success');
            }
            // === 🔬 백업 관련 상세 메시지 처리 ===
    ...
    _handleAckNotification(event) {
        try {
            ...
            // 펌웨어 버전 메시지 특별 디버깅
            if (message.includes('FIRMWARE_VERSION')) {
                console.log('🔍 [FIRMWARE_VERSION_DEBUG] 펌웨어 버전 관련 메시지 감지:', message);
            }
            ...
            // === 🔬 백업 및 업그레이드 관련 메시지 처리 ===
                     ...
                     message.startsWith('FIRMWARE_VERSION')) {
