...
const SOIL_CHARACTERISTIC = '11112222-3333-4444-5555-666677778899';
const RAIN_CHARACTERISTIC = '11112222-3333-4444-5555-66667777889a';
...
const CHARACTERISTIC_UUIDS = {
    ...
    soil: SOIL_CHARACTERISTIC,  // 토양수분센서 추가
    rain: RAIN_CHARACTERISTIC,  // 빗방울센서 추가
    // REPL 모드 특성
...
const SERVICE_CHARACTERISTICS = {
    ...
    [SENSOR_SERVICE]: [ULTRA_CHARACTERISTIC, DHT_CHARACTERISTIC, SERVO_CHARACTERISTIC,
        ...
        BUZZER_CHARACTERISTIC, GYRO_CHARACTERISTIC, DUST_CHARACTERISTIC, DCMOTOR_CHARACTERISTIC, HEART_CHARACTERISTIC, SOIL_CHARACTERISTIC, RAIN_CHARACTERISTIC],
...
window.SOIL_CHARACTERISTIC = SOIL_CHARACTERISTIC;
window.RAIN_CHARACTERISTIC = RAIN_CHARACTERISTIC;
...
/**
 * 토양수분센서 클래스
 * YL-69 토양수분센서를 제어하는 클래스
 */
class SoilMoistureSensor extends SensorBase {
    constructor() {
        super('SoilMoisture', SOIL_CHARACTERISTIC);
        this.moisture = null;    // 수분 백분율 (0-100%)
        this.voltage = null;     // 전압 (V)
        this.rawValue = null;    // 원시 ADC 값
    }

    /**
     * 수분 백분율 반환
     * @returns {number|null} - 수분 백분율 (0-100%) 또는 null
     */
    getMoisture() {
     ...
     */
    async getStatus() {
        console.log("토양수분센서 상태 요청 전송");
        return await this.sendCommand("SOIL:STATUS");
    }
     ...
     */
    async setPin(pin) {
        return await this.sendCommand(`SOIL:PIN:${pin}`);
    }
     ...
     */
    async calibrateDry() {
        console.log("토양수분센서 건조 상태 보정 시작");
        return await this.sendCommand("SOIL:CALIBRATE:DRY");
    }
     ...
     */
    async calibrateWet() {
        console.log("토양수분센서 습윤 상태 보정 시작");
        return await this.sendCommand("SOIL:CALIBRATE:WET");
    }
    ...
    _processData(data) {
        ...
        console.log("[SoilMoistureSensor] 데이터 수신 확인:", data);

        // 데이터 형식 확인 - data.data 구조 처리 추가
        let actualData = data;
        if (data && typeof data === 'object' && data.data !== undefined) {
            console.log("[SoilMoistureSensor] data.data 구조 감지, 내부 데이터 사용");
            actualData = data.data;  // 중첩된 구조 처리
        }
        ...
        try {
            ...
            if (actualData instanceof ArrayBuffer) {
                const decoder = new TextDecoder('utf-8');
                dataStr = decoder.decode(new DataView(actualData));
                console.log("[SoilMoistureSensor] ArrayBuffer에서 변환된 문자열:", dataStr);
            } else if (actualData instanceof DataView) {
                const decoder = new TextDecoder('utf-8');
                dataStr = decoder.decode(actualData);
                console.log("[SoilMoistureSensor] DataView에서 변환된 문자열:", dataStr);
            } else if (actualData && actualData.buffer instanceof ArrayBuffer) {
                // TypedArray 처리 (Uint8Array 등)
                const decoder = new TextDecoder('utf-8');
                dataStr = decoder.decode(actualData);
                console.log("[SoilMoistureSensor] TypedArray에서 변환된 문자열:", dataStr);
            } else {
                // 문자열이거나 다른 타입으로 안전하게 변환
                dataStr = String(actualData);
                console.log("[SoilMoistureSensor] 기본 변환 문자열:", dataStr);
            }
        } catch (e) {
            console.error("[SoilMoistureSensor] 데이터 변환 오류:", e);
            dataStr = String(actualData || "");
            console.log("[SoilMoistureSensor] 오류 후 강제 변환 문자열:", dataStr);
        }

        // 토양수분센서 값 파싱
        let moisture = null, voltage = null, rawValue = null;

        try {
            // 1. "SOIL:75.2,2.456,1250" 형식 처리
            if (dataStr.startsWith("SOIL:") && dataStr.includes(",")) {
                const valueStr = dataStr.substring(5); // "SOIL:" 제거
                const parts = valueStr.split(",");
                if (parts.length >= 3) {
                    moisture = parseFloat(parts[0]);
                    voltage = parseFloat(parts[1]);
                    rawValue = parseInt(parts[2]);
                    console.log(`[SoilMoistureSensor] 파싱: 수분=${moisture}%, 전압=${voltage}V, 원시값=${rawValue}`);
                }
            }

            // 2. 보정 응답 처리 (예: "SOIL:CALIBRATE:DRY:OK:1750")
            else if (dataStr.includes("CALIBRATE")) {
                if (dataStr.includes("DRY:OK:")) {
                    const dryValue = dataStr.match(/DRY:OK:(\d+)/);
                    if (dryValue) {
                        console.log(`[SoilMoistureSensor] 건조 보정 완료: ${dryValue[1]}`);
                    }
                } else if (dataStr.includes("WET:OK:")) {
                    const wetValue = dataStr.match(/WET:OK:(\d+)/);
                    if (wetValue) {
                        console.log(`[SoilMoistureSensor] 습윤 보정 완료: ${wetValue[1]}`);
                    }
                }
            }

            // 값이 유효한지 확인하고 저장
            if (moisture !== null && voltage !== null && rawValue !== null) {
                this.moisture = moisture;
                this.voltage = voltage;
                this.rawValue = rawValue;
                console.log(`[SoilMoistureSensor] 값 업데이트: ${moisture}%, ${voltage}V, ${rawValue}`);
            } else if (!dataStr.includes("CALIBRATE") && !dataStr.includes("ERROR")) {
                console.warn("[SoilMoistureSensor] 토양수분센서 데이터 파싱 실패:", dataStr);
            }
        } catch (e) {
            console.error("[SoilMoistureSensor] 토양수분센서 데이터 파싱 오류:", e);
        }
    }
}

/**
 * 토양수분센서에서 값을 가져오는 함수 (재시도 로직 포함)
 * @param {SoilMoistureSensor} sensor - 토양수분센서 객체
 * @param {number} maxRetries - 최대 재시도 횟수 (기본값: 3)
 * @param {number} retryDelay - 재시도 간격 (밀리초, 기본값: 50)
 * @returns {Promise<{moisture: number|null, voltage: number|null, rawValue: number|null}|null>} 토양수분 값 객체 또는 null
 */
function getSoilMoistureValue(sensor, maxRetries = 3, retryDelay = 50) {
    let retries = 0;

    function tryGetValue() {
        // 센서가 null이거나 undefined인 경우 처리
        if (!sensor) {
            console.log("토양수분센서 객체가 없습니다");
            return Promise.resolve(null);
        }

        return sensor.getStatus().then(() => {
            // 메소드 존재 여부 확인
            const hasMoisture = typeof sensor.getMoisture === 'function';
            const hasVoltage = typeof sensor.getVoltage === 'function';
            const hasRawValue = typeof sensor.getRawValue === 'function';

            if (!hasMoisture && !hasVoltage && !hasRawValue) {
                console.log("토양수분센서에 필요한 메소드가 없습니다");
                return null;
            }

            // 측정값 가져오기
            const moisture = hasMoisture ? sensor.getMoisture() : null;
            const voltage = hasVoltage ? sensor.getVoltage() : null;
            const rawValue = hasRawValue ? sensor.getRawValue() : null;

            // 모든 값이 null이고 재시도 횟수가 남아있으면 재시도
            if ((moisture === null && voltage === null && rawValue === null) && retries < maxRetries) {
                retries++;
                console.log(`토양수분센서 값이 null, 재시도 ${retries}/${maxRetries}`);
            ...
            // 유효한 값이나 최대 재시도 후 최종 값 반환
            return { moisture, voltage, rawValue };
        });
    }

    // 실행 및 오류 처리
    return tryGetValue().catch((error) => {
        console.log("토양수분센서 데이터 요청 중 오류 발생:", error);
        return null;
    });
}
 ...
 * 유효한 토양수분 값 가져오기
 * @param {SoilMoistureSensor} sensor - 토양수분센서 객체
 * @returns {Promise<{moisture: number, voltage: number, rawValue: number}>} 토양수분 값 객체
 */
async function getValidSoilMoistureValue(sensor) {
    try {
        const soilData = await getSoilMoistureValue(sensor);
        return soilData;
    } catch (error) {
        console.error('Error getting valid soil moisture value:', error);
        throw error;
    }
}
...
/**
 * 빗방울센서 클래스 - 저항 방식 빗방울 감지 센서
 */
class RainSensor extends SensorBase {
    constructor() {
        super('RainSensor', RAIN_CHARACTERISTIC);
        this.rainPercentage = null;  // 빗방울 감지 백분율 (0-100%)
        this.voltage = null;         // 전압 (V)
        this.rawValue = null;        // 원시 ADC 값
    }
    ...
    /**
     * 빗방울 감지 백분율 반환 (0-100%)
     * @returns {number|null} 빗방울 감지 백분율
     */
    getRainPercentage() {
    ...
    /**
     * 원시 ADC 값 반환
     * @returns {number|null} 원시 ADC 값 (0-4095)
     */
    getRawValue() {
        return this.rawValue;
    }
    ...
    /**
     * 빗방울 감지 상태 반환
     * @returns {string} 'DRY', 'MOIST', 'WET' 중 하나
     */
    getRainStatus() {
        if (this.rainPercentage === null) return 'UNKNOWN';
        
        if (this.rainPercentage <= 10) return 'DRY';
        else if (this.rainPercentage >= 80) return 'WET';
        else return 'MOIST';
    }
     ...
     */
    async getStatus() {
        console.log("빗방울센서 상태 요청 전송");
        return await this.sendCommand("RAIN:STATUS");
    }
     ...
     */
    async setPin(pin) {
        console.log(`빗방울센서 핀 설정: ${pin}`);
        return await this.sendCommand(`RAIN:PIN:${pin}`);
    }

    /**
     * 센서 데이터 처리 (ESP32에서 수신된 데이터 파싱)
     * 형식: "RAIN:percentage,voltage,rawValue"
     * @param {string} data - 수신된 데이터
     */
    _processData(data) {
        if (!data) return;

        console.log("[RainSensor] 데이터 수신 확인:", data);

        // 데이터 형식 확인 - data.data 구조 처리 추가
        let actualData = data;
        if (data && typeof data === 'object' && data.data !== undefined) {
            console.log("[RainSensor] data.data 구조 감지, 내부 데이터 사용");
            actualData = data.data;  // 중첩된 구조 처리
        }
        ...
        try {
            ...
            if (actualData instanceof ArrayBuffer) {
                const decoder = new TextDecoder('utf-8');
                dataStr = decoder.decode(new DataView(actualData));
                console.log("[RainSensor] ArrayBuffer에서 변환된 문자열:", dataStr);
            } else if (actualData instanceof DataView) {
                const decoder = new TextDecoder('utf-8');
                dataStr = decoder.decode(actualData);
                console.log("[RainSensor] DataView에서 변환된 문자열:", dataStr);
            } else if (actualData && actualData.buffer instanceof ArrayBuffer) {
                // TypedArray 처리 (Uint8Array 등)
                const decoder = new TextDecoder('utf-8');
                dataStr = decoder.decode(actualData);
                console.log("[RainSensor] TypedArray에서 변환된 문자열:", dataStr);
            } else {
                // 문자열이거나 다른 타입으로 안전하게 변환
                dataStr = String(actualData);
                console.log("[RainSensor] 기본 변환 문자열:", dataStr);
            }
        } catch (e) {
            console.error("[RainSensor] 데이터 변환 오류:", e);
            dataStr = String(actualData || "");
            console.log("[RainSensor] 오류 후 강제 변환 문자열:", dataStr);
        }

        dataStr = dataStr.trim();
        console.log(`[RainSensor] 최종 처리할 문자열: "${dataStr}"`);

        // RAIN: 접두사 확인
        if (dataStr.startsWith("RAIN:")) {
            const content = dataStr.substring(5); // "RAIN:" 제거
            console.log(`[RainSensor] RAIN: 접두사 제거 후: "${content}"`);
            
            // 오류 메시지 처리
            if (content.includes("ERROR")) {
                console.error(`[RainSensor] 오류: ${content}`);
                return;
            }

            // 핀 설정 완료 메시지 처리
            if (content.startsWith("PIN:OK:")) {
                const pin = content.split(":")[2];
                console.log(`[RainSensor] 핀 설정 완료: ${pin}`);
                return;
            }

            try {
                // 데이터 파싱: "percentage,voltage,rawValue"
                const values = content.split(",");
                console.log(`[RainSensor] 분할된 값들:`, values);
                if (values.length >= 3) {
                    const percentage = parseFloat(values[0]);
                    const voltage = parseFloat(values[1]);
                    const rawValue = parseInt(values[2]);

                    console.log(`[RainSensor] 파싱된 값들: percentage=${percentage}, voltage=${voltage}, rawValue=${rawValue}`);

                    // 값이 유효한지 확인하고 저장
                    if (!isNaN(percentage) && !isNaN(voltage) && !isNaN(rawValue)) {
                        this.rainPercentage = percentage;
                        this.voltage = voltage;
                        this.rawValue = rawValue;
                        console.log(`[RainSensor] 값 업데이트: ${percentage}%, ${voltage}V, ${rawValue}`);
                    } else {
                        console.warn("[RainSensor] 빗방울센서 데이터 파싱 실패:", dataStr);
                        console.warn(`[RainSensor] 파싱 실패 상세: percentage=${percentage}, voltage=${voltage}, rawValue=${rawValue}`);
                    }
                } else {
                    console.warn(`[RainSensor] 값 개수 부족: ${values.length}개 (최소 3개 필요)`);
                }
            } catch (error) {
                console.error("[RainSensor] 데이터 처리 오류:", error, "원본 데이터:", dataStr);
            }
        } else {
            console.warn(`[RainSensor] RAIN: 접두사가 없는 데이터: "${dataStr}"`);
        }
    }
}

/**
 * 빗방울센서에서 값을 가져오는 함수 (재시도 로직 포함)
 * @param {RainSensor} sensor - 빗방울센서 객체
 * @param {number} maxRetries - 최대 재시도 횟수 (기본값: 3)
 * @param {number} retryDelay - 재시도 간격 (밀리초, 기본값: 50)
 * @returns {Promise<{rainPercentage: number|null, voltage: number|null, rawValue: number|null}|null>} 빗방울 값 객체 또는 null
 */
function getRainSensorValue(sensor, maxRetries = 3, retryDelay = 50) {
    let retries = 0;

    function tryGetValue() {
        // 센서가 null이거나 undefined인 경우 처리
        if (!sensor) {
            console.log("빗방울센서 객체가 없습니다");
            return Promise.resolve(null);
        }

        return sensor.getStatus().then(() => {
            console.log('[getRainSensorValue] getStatus 완료');
            
            // 메소드 존재 여부 확인
            const hasRainPercentage = typeof sensor.getRainPercentage === 'function';
            const hasVoltage = typeof sensor.getVoltage === 'function';
            const hasRawValue = typeof sensor.getRawValue === 'function';

            console.log('[getRainSensorValue] 메소드 존재 여부:', {
                hasRainPercentage, hasVoltage, hasRawValue
            });

            if (!hasRainPercentage && !hasVoltage && !hasRawValue) {
                console.log("빗방울센서에 필요한 메소드가 없습니다");
                return null;
            }

            // 측정값 가져오기
            const rainPercentage = hasRainPercentage ? sensor.getRainPercentage() : null;
            const voltage = hasVoltage ? sensor.getVoltage() : null;
            const rawValue = hasRawValue ? sensor.getRawValue() : null;

            console.log('[getRainSensorValue] 센서에서 가져온 값들:', {
                rainPercentage, voltage, rawValue
            });

            // 모든 값이 null이고 재시도 횟수가 남아있으면 재시도
            if ((rainPercentage === null && voltage === null && rawValue === null) && retries < maxRetries) {
                retries++;
                console.log(`빗방울센서 값이 null, 재시도 ${retries}/${maxRetries}`);
            ...
            // 유효한 값이나 최대 재시도 후 최종 값 반환
            const result = { rainPercentage, voltage, rawValue };
            console.log('[getRainSensorValue] 최종 반환 값:', result);
            return result;
        });
    }
    ...
    // 실행 및 오류 처리
    return tryGetValue().catch((error) => {
        console.log("빗방울센서 데이터 요청 중 오류 발생:", error);
        return null;
    });
}

/**
 * 빗방울센서에서 유효한 값을 가져오는 함수 (getRainSensorValue 활용)
 * @param {RainSensor} sensor - 빗방울센서 객체
 * @returns {Promise<{rainPercentage: number|null, voltage: number|null, rawValue: number|null}|null>}
 */
async function getValidRainSensorValue(sensor) {
    try {
        console.log('[getValidRainSensorValue] 시작 - 센서 객체:', sensor);
        const rainData = await getRainSensorValue(sensor);
        console.log('[getValidRainSensorValue] getRainSensorValue 결과:', rainData);
        
        if (rainData) {
            console.log('[getValidRainSensorValue] 상세 데이터:');
            console.log('  - rainPercentage:', rainData.rainPercentage);
            console.log('  - voltage:', rainData.voltage);
            console.log('  - rawValue:', rainData.rawValue);
        }
        
        return rainData;
    } catch (error) {
        console.error('Error getting valid rain sensor value:', error);
        throw error;
    }
}
// ... existing code ...
...
window.SoilMoistureSensor = SoilMoistureSensor;  // SoilMoistureSensor 클래스 추가
window.RainSensor = RainSensor;  // RainSensor 클래스 추가
...
window.getSoilMoistureValue = getSoilMoistureValue; // 토양수분센서 함수 추가
window.getRainSensorValue = getRainSensorValue; // 빗방울센서 함수 추가
...
window.getValidSoilMoistureValue = getValidSoilMoistureValue; // 토양수분센서 유효값 함수 추가
window.getValidRainSensorValue = getValidRainSensorValue; // 빗방울센서 유효값 함수 추가
...
// 
// // 핀 설정
// await soilSensor.setPin(2);  // 토양수분센서를 ADC 핀 2에 연결
// 
// // 측정
// await soilSensor.getStatus();  // 토양수분 측정 요청
// const moisture = soilSensor.getMoisture();  // 수분 백분율 가져오기 (0-100%)
// const voltage = soilSensor.getVoltage();    // 전압 값 가져오기 (V)
// const raw = soilSensor.getRawValue();       // 원시 ADC 값 가져오기
// 
// // 보정
// await soilSensor.calibrateDry();  // 건조 상태 보정 (센서를 공기중에 두고 실행)
// await soilSensor.calibrateWet();  // 습윤 상태 보정 (센서를 물에 담그고 실행)
// 
// // 유틸리티 함수 사용
// const soilData = await getValidSoilMoistureValue(soilSensor);
// console.log(`수분: ${soilData.moisture}%, 전압: ${soilData.voltage}V, 원시값: ${soilData.rawValue}`);
...
// 
// // 핀 설정
// await rainSensor.setPin(3);  // 빗방울센서를 ADC 핀 3에 연결
// 
// // 측정
// await rainSensor.getStatus();  // 빗방울 측정 요청
// const rainPercentage = rainSensor.getRainPercentage();  // 빗방울 감지 백분율 가져오기 (0-100%)
// const voltage = rainSensor.getVoltage();    // 전압 값 가져오기 (V)
// const raw = rainSensor.getRawValue();       // 원시 ADC 값 가져오기
// const status = rainSensor.getRainStatus();  // 'DRY', 'MOIST', 'WET' 상태
// 
// // 유틸리티 함수 사용
// const rainData = await getValidRainSensorValue(rainSensor);
// console.log(`빗방울: ${rainData.rainPercentage}%, 전압: ${rainData.voltage}V, 원시값: ${rainData.rawValue}`);
