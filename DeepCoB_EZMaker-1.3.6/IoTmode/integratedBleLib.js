...
class SensorBase { 
  ...
  _setupNotificationsAfterConnection() {
    ...
    try {
      ...
      this.bleManager.startNotifications(this.characteristicUUID, function(data) {
        ...
        console.error(`[DEBUG] ${self.name}: 특성 알림 시작 실패:`, error);
      });
  ...
  async sendCommand(command) {
    ...
    try {
      return await this.bleManager.sendCommand(this.characteristicUUID, command);
    } catch (error) {
...
class ServoMotor extends SensorBase {
  ...
  constructor() {
    ...
    this.currentAngle = 0; // 현재 각도 저장용 멤버 변수
  }
  ...
  _processTextCommand(text) {
    ...
    if (text.startsWith("CAM:START")) {
      ...
      try {
        ...
        this.expectedChunks = Math.ceil(this.currentFrameSize / 160);
      } catch (e) {
...
function getLightSensorValue(sensor, maxRetries = 3, retryDelay = 50) {
    ...
    function tryGetValue() {
        ...
        if (!sensor) {
            console.log("센서 객체가 없습니다");
            return Promise.reject(new Error("센서 객체가 없습니다"));
        }
        ...
        return sensor.getStatus().then(() => {
            ...
            if (typeof sensor.getAnalog === 'function') {
                ...
                // 유효한 값만 반환, 그렇지 않으면 오류 발생
                if (value !== null && value !== undefined) {
                    return value;
                } else {
                    return Promise.reject(new Error("유효한 센서 값을 얻을 수 없습니다"));
                }
            } else {
                console.log("센서에 getAnalog 메소드가 없습니다");
                return Promise.reject(new Error("센서에 getAnalog 메소드가 없습니다"));
            }
        });
    }
...
async function getValidUltrasonicValue(sensor) {
    const distance = await getUltrasonicSensorValue(sensor);
...
window.Camera = Camera;  // 카메라 클래스 추가
// 전역 함수 추가
...
class DustSensor extends SensorBase {
  ...
  constructor() {
    ...
    this.isCalibrated = false; // 보정 완료 여부
  }
...
function getDustSensorValue(sensor, maxRetries = 3, retryDelay = 50) {
    ...
    // 실행 및 오류 처리
    return tryGetValue().catch((error) => {
        console.log("먼지 센서 데이터 요청 중 오류 발생:", error);
        return null;
    });
}
...
class NeoPixel extends SensorBase {
   ...
   * @returns {Promise<boolean>} - 성공 여부
   */
  async setPixelColor(index, r, g, b) {
    // 색상 값 범위 확인 및 조정
  ...
  async setAllPixels(r, g, b) {
    ...
    try {
      const result = await this.sendCommand(`NEO:SETALL:${r},${g},${b}`);
