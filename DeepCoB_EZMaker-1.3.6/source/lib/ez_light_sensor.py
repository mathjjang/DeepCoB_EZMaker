"""
EZMaker 전용 밝기센서(EZLIGHT) 드라이버

- EZLIGHT 스펙(교육용 기준, EZMaker 사이트 스타일 가정):
...
기존 공통 조도센서(LIGHT)와 처리 방식을 분리하기 위해
EZ 전용 센서로 별도 클래스를 제공합니다.
"""
...
class EzLightSensor:
    """
    EZMaker 밝기센서(EZLIGHT) 드라이버

    사용 예:
        sensor = EzLightSensor(adc_pin=2)  # 예: A0 포트
        status = sensor.get_status()
        print(status["raw"], status["percent"])
    """
...
if __name__ == "__main__":
    print("EzLightSensor 드라이버 모듈입니다.")
    print("예시:")
    print("  from ez_light_sensor import EzLightSensor")
    print("  sensor = EzLightSensor(adc_pin=2)  # A0 포트")
