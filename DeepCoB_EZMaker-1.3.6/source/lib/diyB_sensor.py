"""
DIY-B 센서 드라이버 (전류 / 전도도 실험용 아날로그 센서)
...
DIY-B 센서는 센서 내부 저항을 통해 "전류/전도도"를 전압으로 변환해 주기 때문에,
실제 펌웨어/블록코드에서는 **전압(0~5V)과 10비트 Raw 값을 읽고, 수업에서 전류 개념으로 해석**하는 방식을 사용합니다.
"""
...
class DiyBSensor:
    """
    DIY-B 아날로그 센서 드라이버 (전류 / 전도도 센서)

    사용 예:
        sensor = DiyBSensor(adc_pin=1)  # A1 포트 (GPIO 1)
...
if __name__ == "__main__":
    print("DiyBSensor 드라이버 모듈입니다.")
    print("예시:")
    print("  from diyB_sensor import DiyBSensor")
    print("  sensor = DiyBSensor(adc_pin=1)  # A1 포트")
    print("  status = sensor.get_status()")
    print("  print(status['raw'], status['voltage'])")
